...
namespace AssignmentNearestThousand
{
    class Program
    {
        static void Main()
        {
            // Input
            int number = 3852;
            // We upcasting (converting) the 'int' input value to 'decimal' type; so that, we can get fractional value while dividing the number below
			
            decimal decimal_number = number;

            // Round down to previous thousand. Means, replace the last three digits with '000'. 
            //'number / 1000' will integer divide (because the 'number' variable is an 'int') with 1000. So you will get '3' in this example. Then multiply the same with 1000. So it becomes '3000'
            int previous_thousand = number / 1000 * 1000;

            // In Next few statements, we are trying to get last three digits from the number. 
            // In this example, the number is 3852. So "decimal_number / 1000" will get "3.852". 
            decimal last_three_digits_x = decimal_number / 1000;

            // "number / 1000" will get "3".
            decimal last_three_digits_y = number / 1000;

            // Then we will subtract these 'x' and 'y'. So "3.852 - 3" will be "0.852". Then we will multiply the same with 1000. So "0.852 * 1000" will be "852". In this way, we are getting the last three digits of the actual number.
            ...
            // Here, the expression "last_three_digits >= 500" returns a boolean value(either true or false); If it is 'true', the 'rounding_value" will be '1000'; otherwise, '0'.
            int rounding_value = (last_three_digits >= 500) ? 1000 : 0;

            // In a different case, if the actual 'number' itself is less than 500 (Eg: 499), we need to round up to 1000 always. Means, any input number less than 500 gives result as '1000'.
            int is_less_than_five_hundred = (number < 500) ? 1000 : 0;

            // Return the sum of all above three values
